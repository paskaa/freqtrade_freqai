...
class Alvinchen_15m131_FreqAI(IStrategy):
    """
    ...
    def set_freqai_targets(self, dataframe: DataFrame, metadata: dict, **kwargs) -> DataFrame:
        ...
        label_period = self.config.get('freqai', {}).get('feature_parameters', {}) \
            .get('label_period_candles', 8)

        dataframe["&-s_close"] = (
            dataframe["close"].shift(-label_period) / dataframe["close"] - 1
        )
        dataframe["&-s_direction"] = np.where(
            dataframe["&-s_close"] > 0.003, 2,
            np.where(dataframe["&-s_close"] < -0.003, 0, 1)
        )
        return dataframe
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        """
        Entry conditions - v20 Optimized
