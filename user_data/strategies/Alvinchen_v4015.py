"""
ADX/DI Strategy - v4015 (SuperTrend Enhanced)
=============================================

基于v4014优化版，添加SuperTrend趋势确认：
...
class Alvinchen_v4015(IStrategy):
    """
    v4015 - SuperTrend Enhanced

    优化点：
    1. 向量化SuperTrend计算
    2. SuperTrend作为趋势确认过滤器
    3. 多时间框架SuperTrend确认
    """
    ...
    buy_adx_tier1 = DecimalParameter(28, 28, default=28.0, decimals=1, optimize=False, space='buy')
    buy_adx_tier2 = DecimalParameter(32, 32, default=32.0, decimals=1, optimize=False, space='buy')
    buy_adx_tier3 = DecimalParameter(35, 35, default=35.0, decimals=1, optimize=False, space='buy')
    buy_adx_tier4 = DecimalParameter(40, 40, default=40.0, decimals=1, optimize=False, space='buy')
    ...
    tier1_initial_stoploss = 0.18
    tier1_profit_stoploss = 0.05
    tier2_initial_stoploss = 0.15
    tier2_profit_stoploss = 0.04
    tier3_initial_stoploss = 0.12
    tier3_profit_stoploss = 0.03
    tier4_initial_stoploss = 0.10
    ...
    def __init__(self, config: dict) -> None:
        ...
        logger.info(f"v4015 SuperTrend Enhanced Strategy - enabled: {self.freqai_enabled}")
    ...
    def _calculate_supertrend_vectorized(self, df: DataFrame, period: int = 10, multiplier: float = 3.0) -> DataFrame:
        """
        向量化SuperTrend计算 - 高效版本

        SuperTrend公式：
        1. 计算ATR
        2. 基础上轨 = HL2 + (multiplier * ATR)
        3. 基础下轨 = HL2 - (multiplier * ATR)
        4. 最终上轨：价格突破上轨时重置，否则取最小值
        5. 最终下轨：价格突破下轨时重置，否则取最大值
        6. 趋势方向：价格在上轨之上为多头，价格在下轨之下为空头
        """
        high = df['high']
        low = df['low']
        close = df['close']
        ...
        direction = pd.Series(1, index=df.index)  # 1=多头, -1=空头
        ...
        supertrend_np = np.zeros(len(df))
        direction_np = np.ones(len(df))

        # 第一根K线初始化
        supertrend_np[0] = upper_band_np[0]
        direction_np[0] = 1

        for i in range(1, len(df)):
            # 上轨调整：只能下降，不能上升（多头市场中）
            if upper_band_np[i] < supertrend_np[i-1] or close_np[i-1] > supertrend_np[i-1]:
                upper_band_np[i] = upper_band_np[i]
            else:
                upper_band_np[i] = supertrend_np[i-1]

            # 下轨调整：只能上升，不能下降（空头市场中）
            if lower_band_np[i] > supertrend_np[i-1] or close_np[i-1] < supertrend_np[i-1]:
                lower_band_np[i] = lower_band_np[i]
            else:
                lower_band_np[i] = supertrend_np[i-1]

            # 趋势判断
            if direction_np[i-1] == 1:  # 之前是多头
                if close_np[i] < lower_band_np[i]:
                    direction_np[i] = -1  # 转为空头
                    supertrend_np[i] = upper_band_np[i]
                else:
                    direction_np[i] = 1  # 保持多头
                    supertrend_np[i] = lower_band_np[i]
            else:  # 之前是空头
                if close_np[i] > upper_band_np[i]:
                    direction_np[i] = 1  # 转为多头
                    supertrend_np[i] = lower_band_np[i]
                else:
                    direction_np[i] = -1  # 保持空头
                    supertrend_np[i] = upper_band_np[i]

        df['st_supertrend'] = supertrend_np
        df['st_direction'] = direction_np
        df['st_bullish'] = direction_np == 1
        df['st_bearish'] = direction_np == -1

        # SuperTrend信号变化点
        df['st_bullish_change'] = (df['st_direction'] == 1) & (df['st_direction'].shift(1) == -1)
        df['st_bearish_change'] = (df['st_direction'] == -1) & (df['st_direction'].shift(1) == 1)
    ...
    def populate_indicators_1h(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # 1h SuperTrend
        dataframe = self._calculate_supertrend_vectorized(dataframe, period=10, multiplier=3.0)
        dataframe['st_bullish_1h'] = dataframe['st_bullish']
        dataframe['st_bearish_1h'] = dataframe['st_bearish']
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # v4015: SuperTrend趋势确认 (向量化) - 放宽条件
        st_bullish = dataframe['st_bullish']  # 15m SuperTrend看多
        st_bearish = dataframe['st_bearish']  # 15m SuperTrend看空

        # SuperTrend方向匹配时降低ADX阈值 (奖励趋势一致)
        st_long_bonus = st_bullish.astype(float) * 3.0  # SuperTrend看多时ADX阈值降低3
        st_short_bonus = st_bearish.astype(float) * 3.0  # SuperTrend看空时ADX阈值降低3

        long_entry_base = (
            major_bullish &
            minor_bullish &
            (dataframe['adx'] > (long_adx_th - st_long_bonus)) &  # SuperTrend一致时降低ADX阈值
        ...
        short_entry_base = (
            major_bearish &
            minor_bearish &
            (dataframe['adx'] > (short_adx_th - st_short_bonus)) &  # SuperTrend一致时降低ADX阈值
        ...
        if has_fib_data:
            ...
            long_entry = (
                major_bullish &
                minor_bullish &
                (dataframe['adx'] > (long_adx_th_adj - st_long_bonus)) &  # v4015: SuperTrend奖励
                (dataframe['di_diff'] > long_di_th) &
                (dataframe['macdhist'] > 0) &
                (dataframe['rsi'] > self.buy_rsi_long_lower.value) &
                (dataframe['rsi'] < self.buy_rsi_long_upper.value) &
            ...
            short_entry = (
                major_bearish &
                minor_bearish &
                (dataframe['adx'] > (short_adx_th_adj - st_short_bonus)) &  # v4015: SuperTrend奖励
                (dataframe['di_diff'] < -short_di_th) &
                (dataframe['macdhist'] < 0) &
                (dataframe['rsi'] < self.buy_rsi_short_upper.value) &
                (dataframe['rsi'] > self.buy_rsi_short_lower.value) &
        ...
        # v4015: 放宽市场方向过滤 - 只在极端情况下禁止反向交易
        # 如果SuperTrend与市场方向一致，允许交易
        # 如果市场是neutral，允许双向交易
        if market_regime == 'bullish':
            # 牛市中，只有当SuperTrend明确看空且EMA方向也看空时才允许做空
            short_entry = short_entry & st_bearish & minor_bearish
        elif market_regime == 'bearish':
            # 熊市中，只有当SuperTrend明确看多且EMA方向也看多时才允许做多
            long_entry = long_entry & st_bullish & minor_bullish
        # neutral: 允许双向交易
        ...
        dataframe.loc[long_entry, 'enter_tag'] = f'v4015_long_{tier_name}_{market_regime}'

        dataframe.loc[short_entry, 'enter_short'] = 1
        dataframe.loc[short_entry, 'enter_tag'] = f'v4015_short_{tier_name}_{market_regime}'
    ...
    def custom_stoploss(
        ...
        **kwargs
        ...
        if tier == 1:
            initial_stoploss = self.tier1_initial_stoploss
        elif tier == 2:
            initial_stoploss = self.tier2_initial_stoploss
        elif tier == 3:
            initial_stoploss = self.tier3_initial_stoploss
        else:
            initial_stoploss = self.tier4_initial_stoploss
    ...
    def custom_exit(
        ...
        **kwargs
        ...
        # v4015: SuperTrend趋势反转退出
        if 'st_bearish_change' in dataframe.columns and not trade.is_short:
            if last_candle.get('st_bearish_change', False):
                return 'supertrend_reversal_exit'

        if 'st_bullish_change' in dataframe.columns and trade.is_short:
            if last_candle.get('st_bullish_change', False):
                return 'supertrend_reversal_exit'
