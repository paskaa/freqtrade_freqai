...
class Alvinchen_v34_20(IStrategy):
    INTERFACE_VERSION = 3
    """
    ...
    # Stoploss parameters - hyperopt 优化后的固定值
    long_stoploss = 0.439       # 多单止损 43.9%
    short_stoploss = 0.329      # 空单止损 32.9%
    ...
    def populate_indicators_1h(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        """1h趋势确认指标"""
        dataframe['ema_20_1h'] = ta.EMA(dataframe['close'], timeperiod=20)
        dataframe['ema_50_1h'] = ta.EMA(dataframe['close'], timeperiod=50)
    ...
    def populate_indicators_4h(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe['close_4h'] = dataframe['close']  # 用于判断价格在EMA上方还是下方
        return dataframe
    ...
    def custom_stoploss(
        ...
        **kwargs
        ...
        使用 hyperopt 参数:
        - long_stoploss: 多单止损比例
        - short_stoploss: 空单止损比例
        - profit_lock_threshold: 利润锁定阈值
        - profit_lock_ratio: 利润锁定比例
        """
        # 根据交易方向设置基础止损 (使用 hyperopt 参数)
        if trade.is_short:
            base_sl = -self.short_stoploss
        else:
            base_sl = -self.long_stoploss

        # 亏损时保持固定止损
        if current_profit < self.profit_lock_threshold:
            return base_sl

        # 盈利时计算移动止损，逐步锁定利润 (使用 hyperopt 参数)
        if current_profit >= 0.12:
            locked_profit = current_profit * (self.profit_lock_ratio + 0.10)
        elif current_profit >= 0.08:
            locked_profit = current_profit * self.profit_lock_ratio
    ...
    def custom_exit(
        ...
        **kwargs
        ...
        """
        v34.13 自定义退出 - 使用 hyperopt 参数

        退出条件：
        1. 必须有盈利 (current_profit > 0)
        2. 信号强烈反转（至少3个反转信号同时满足）
        ...
        Hyperopt 参数:
        - exit_rsi_overbought: RSI超买阈值
        - exit_rsi_oversold: RSI超卖阈值
        """
        # 不盈利不退出，让ROI和trailing stop处理
        if current_profit <= 0:
            return None
