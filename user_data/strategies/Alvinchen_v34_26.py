"""
ADX/DI Strategy without FreqAI - v34.20 (Multi-Timeframe Analysis)
===============================================================

基于 v34_19 的 sentiment filter 优化 + 多时间框架分析。

v34.20 关键改进:
1. 使用 @informative 装饰器实现多时间框架:
   - 1h: EMA20, EMA50, ADX 趋势确认
   - 4h: EMA50, ADX 方向过滤
2. balance_check_threshold: 0.75
3. sentiment_allow_contrarian_shorts: True

MTF逻辑:
- 做多: 15m信号 + 1h EMA20>EMA50 + 4h ADX>25
- 做空: 15m信号 + 1h EMA20<EMA50 + 4h ADX>25
...
class Alvinchen_v34_26(IStrategy):
    INTERFACE_VERSION = 3
    """
    ADX/DI Strategy without FreqAI - v34.26 (Multi-Timeframe)

    Key features:
    - No FreqAI (pure technical analysis)
    - ADX/DI entry conditions with Multi-Timeframe confirmation
    - 1h/4h EMA and ADX for trend filtering
    ...
    timeframe = '15m'
    startup_candle_count = 1000  # 需要足够多以计算1d EMA200

    @property
    def informative_pairs(self):
        """指定需要获取多时间框架数据的交易对"""
        # 注意: 不再这里定义1d，因为某些交易对可能没有1d数据
        # 改为在populate_indicators_1d中处理
        whitelist = self.config.get('exchange', {}).get('pair_whitelist', [])
        return [(pair, '1h') for pair in whitelist]

    # ==================== FIXED OPTIMIZED PARAMETERS (Hyperopt Epoch 40) ====================
    # Entry parameters - hyperopt 优化后的固定值
    adx_threshold = 19.788      # ADX入场阈值
    volume_mult = 1.221         # 成交量倍数

    # Exit parameters
    exit_rsi_overbought = 75    # RSI超买阈值
    exit_rsi_oversold = 35      # RSI超卖阈值

    # Stoploss parameters - 宽松初始止损，动态收紧
    long_stoploss = 0.50        # 多单初始止损 50%
    short_stoploss = 0.50       # 空单初始止损 50%

    # ==================== 币种分级配置 ====================
    # 主流币种: 流动性好、波动相对平稳、订单簿厚
    major_coins = ['BTC/USDT', 'ETH/USDT', 'BNB/USDT', 'XRP/USDT', 'SOL/USDT']
    # 二流币种: 流动性较差、波动大、容易被操纵
    minor_coins = ['DOGE/USDT', 'ADA/USDT', 'NEAR/USDT', 'LINK/USDT', 'AVAX/USDT',
                   'DOT/USDT', 'LTC/USDT', 'ATOM/USDT', 'ARB/USDT', 'OP/USDT']

    # 币种分级指标阈值
    # 主流币种可以使用更严格的指标
    major_adx_threshold = 30      # 主流币 ADX 阈值
    major_di_threshold = 5       # 主流币 DI_diff 阈值
    major_volume_ratio = 1.2     # 主流币成交量倍数
    # 二流币种需要更明确的趋势才入场
    minor_adx_threshold = 35      # 二流币 ADX 阈值 (更严格)
    minor_di_threshold = 8       # 二流币 DI_diff 阈值 (更严格)
    minor_volume_ratio = 1.5     # 二流币成交量倍数 (更高要求)

    # Profit lock parameters
    profit_lock_threshold = 0.052   # 利润锁定阈值 5.2%
    profit_lock_ratio = 0.761       # 利润锁定比例 76.1%
    ...
    # v34.19 新增: 情绪过滤器修复
    sentiment_allow_contrarian_shorts = True  # 允许在极度恐慌时做空（强趋势时）
    sentiment_contrarian_adx_threshold = 30.0  # 允许逆势做空的最低ADX阈值
    balance_check_threshold = 0.75        # 多空平衡检查阈值 (原为0.5，现改为0.75)
    ...
    # ========== V31: 放宽止损配置 ==========
    # 止损由 hyperopt 参数控制 (见上方 long_stoploss, short_stoploss)
    ...
    # 利润锁定参数由 hyperopt 控制 (见上方 profit_lock_threshold, profit_lock_ratio)

    # ========== V31: 补仓策略配置 ==========
    position_adjustment_enable = False     # 禁用补仓
    max_entry_position_adjustment = -1     # 禁用补仓
    ...
    # ========== Multi-Timeframe Indicators (v34.20) ==========
    # 只使用1h作为中期确认 (避免1d数据缺失问题)
    @informative('1h')
    def populate_indicators_1h(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        """
        1h中期趋势确认指标

        MTF三层模型 (使用1h替代1d避免数据缺失):
        - Layer 1 (1d替代): 通过1h EMA200判断整体趋势 (使用更长的回溯周期)
        - Layer 2 (1h): 中期趋势确认
        - Layer 3 (15m): 精确入场点
        """
        # EMA排列判断趋势方向 (使用更长周期模拟日线效果)
        dataframe['ema_20'] = ta.EMA(dataframe['close'], timeperiod=20)
        dataframe['ema_50'] = ta.EMA(dataframe['close'], timeperiod=50)
        dataframe['ema_200'] = ta.EMA(dataframe['close'], timeperiod=200)
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        """
        Entry conditions - v34.22: 启用多空双方向

        核心原则: "Better to miss than to make mistakes"
        - 只在高度确信时入场
        - 多指标确认才入场
        - 趋势方向必须明确

        MTF三层模型:
        - Layer 1 (1h大周期): EMA50 > EMA200 确认趋势方向
        - Layer 2 (1h中周期): EMA20 > EMA50 确认动量
        - Layer 3 (15m小周期): 多指标确认入场点

        币种分级:
        - 主流币种: 流动性好，可以使用较灵敏的指标
        - 二流币种: 流动性差，需要更明确的信号
        """
        # ========== 获取币种分级和对应阈值 ==========
        pair = metadata.get('pair', '')
        # 提取币种名称 (去掉 /USDT:USDT 后缀)
        coin_base = pair.split('/')[0] if '/' in pair else pair

        # 判断币种分级并选择阈值
        if coin_base in ['BTC', 'ETH', 'BNB', 'XRP', 'SOL']:
            # 主流币种
            is_major = True
            adx_threshold = 30
            di_threshold = 5
        ...
        # ========== 15m精确入场指标 (使用分级阈值) ==========
        # 1. ADX趋势强度
        adx_strong = dataframe['adx'] > adx_threshold

        # 2. DI动量差值
        di_bullish = dataframe['di_diff'] > di_threshold
        di_bearish = dataframe['di_diff'] < -di_threshold

        # 3. MACD 动量方向
        macd_bullish = dataframe['macdhist'] > 0
        macd_bearish = dataframe['macdhist'] < 0

        # 4. RSI 动能 - 做空需要明确在下跌过程中
        rsi_bullish = (dataframe['rsi'] > 55) & (dataframe['rsi'] < 70)
        rsi_bearish = (dataframe['rsi'] < 45) & (dataframe['rsi'] > 20)

        # 5. 成交量确认 (使用分级阈值)
        volume_above_avg = dataframe['volume_ratio'] > volume_ratio_threshold

        # 6. 避免在过度反弹时入场
        rsi_not_oversold = dataframe['rsi'] > 25

        # ========== 高准确性入场条件 ==========
        # 做多: 极严格条件 - 只在明确上升趋势中做多
        # v34.26: 继续提高阈值
        long_entry = (
            major_bullish &             # 大周期多头 (必须)
            minor_bullish &            # 小周期多头 (必须)
            (dataframe['adx'] > 50) &   # ADX > 50 极强上升趋势确认 (原45)
            (dataframe['di_diff'] > 18) &  # DI_diff > 18 极强多头 (原15)
            macd_bullish &             # MACD > 0 确认多头
            (dataframe['rsi'] > 60) &   # RSI > 60 明确上升趋势
            (dataframe['rsi'] < 65) &   # RSI < 65 避免过热
            volume_above_avg &         # 成交量放量确认
            (dataframe['volume'] > 0)
        )

        # 做空: 保持原有严格条件
        short_entry = (
            major_bearish &             # 大周期空头 (必须)
            minor_bearish &            # 小周期空头 (必须)
            (dataframe['adx'] > 35) &   # ADX > 35 强趋势确认
            (dataframe['di_diff'] < -8) &  # DI_diff < -8 明确空头
            macd_bearish &             # MACD < 0 确认空头
            (dataframe['rsi'] < 45) &   # RSI < 45 明确弱势
            (dataframe['rsi'] > 20) &   # RSI > 20 避免超卖反弹
            volume_above_avg &         # 成交量放量确认
            (dataframe['volume'] > 0)
        )
        ...
        dataframe.loc[long_entry, 'enter_tag'] = f'mtf_long{"_major" if is_major else "_minor"}'

        dataframe.loc[short_entry, 'enter_short'] = 1
        dataframe.loc[short_entry, 'enter_tag'] = f'mtf_short{"_major" if is_major else "_minor"}'

        return dataframe

    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        """
        Exit signals - v34.8: 禁用标准退出信号

        退出逻辑已移至 custom_exit 方法：
        - 只有盈利且信号强烈反转才退出
        - 亏损时不触发退出信号

        这样可以避免假信号导致的过早退出
        """
        # 禁用标准退出信号，使用 custom_exit 处理
    ...
    def custom_exit(
        ...
        **kwargs
        ...
        退出条件：
        1. 盈利时: 3个反转信号退出
        2. 小幅亏损(-3%以内)时: 4个反转信号（全部指标都反转）退出
        3. 大幅亏损超过-3%: 需要4个信号 + ADX<25（趋势反转确认）

        反转信号：
        - 多单：EMA死叉 + MACD死叉 + DI反转 + RSI超买回落
        - 空单：EMA金叉 + MACD金叉 + DI反转 + RSI超卖回升
        """
        # 获取当前K线数据
        ...
        if not trade.is_short:
            ...
            # 4. RSI超买回落 (使用 hyperopt 参数)
            if last_candle["rsi"] > self.exit_rsi_overbought:
                reverse_signals += 1

            # 盈利时3个信号退出
            if current_profit > 0 and reverse_signals >= 3:
                return 'profit_reverse_signal'

            # 小幅亏损(-3%以内)时4个信号退出
            if -0.03 < current_profit <= 0 and reverse_signals >= 4:
                return 'small_loss_exit'

            # 大幅亏损(-3%以上)时4个信号+ADX<25退出
            if current_profit <= -0.03 and reverse_signals >= 4 and last_candle["adx"] < 25:
        ...
        # 空单退出检查
            ...
            # 4. RSI超卖回升 (使用 hyperopt 参数)
            if last_candle["rsi"] < self.exit_rsi_oversold:
                reverse_signals += 1
        ...
        **kwargs
        ...
        if trades:
            ...
            if side == 'long' and long_count >= max_per_direction:
                logger.info(f"⚖️ [多空平衡拒绝] {pair} {side}: 多单数量已达上限 ({long_count}/{max_per_direction})")
                return False

            if side == 'short' and short_count >= max_per_direction:
                logger.info(f"⚖️ [多空平衡拒绝] {pair} {side}: 空单数量已达上限 ({short_count}/{max_per_direction})")
                return False

            # 额外检查: 资金平衡 (如果总订单数超过阈值)
            total_count = long_count + short_count
            if total_count >= max_per_direction * 1.5:  # 当总订单超过单个方向上限的1.5倍时检查资金
                long_stake = sum(t.stake_amount for t in long_trades)
                short_stake = sum(t.stake_amount for t in short_trades)
                total_stake = long_stake + short_stake
                threshold = self.balance_check_threshold
                if side == 'long' and (long_stake > total_stake * threshold):
                    logger.info(f"⚖️ [多空平衡拒绝] {pair} {side}: 多单资金占比过高 (资金:{long_stake:.2f}/{total_stake:.2f})")
                    return False
                if side == 'short' and (short_stake > total_stake * threshold):
                    logger.info(f"⚖️ [多空平衡拒绝] {pair} {side}: 空单资金占比过高 (资金:{short_stake:.2f}/{total_stake:.2f})")
                    return False
        ...
        # ========== 情绪分析检查 (仅实盘) ==========
        sentiment_allowed, sentiment_reason = self.check_sentiment_filter(pair, side, last_candle)
    ...
    def check_sentiment_filter(self, pair: str, side: str, last_candle=None) -> tuple:
        """
        情绪分析过滤 (v34.19 修复版)

        改进: 允许在极度恐慌时做空，如果ADX很高（强趋势）
        ...
        try:
            ...
            # v34.19: 极度恐慌时不再完全禁止做空
            if fng <= self.sentiment_extreme_fear:
                if side == 'short':
                    # 检查是否可以逆势做空
                    if self.sentiment_allow_contrarian_shorts and last_candle is not None:
                        adx = last_candle.get('adx', 0)
                        if adx > self.sentiment_contrarian_adx_threshold:
                            logger.info(f"📈 [情绪信号] 极度恐慌(F&G={fng})但ADX={adx:.1f}强, 允许做空")
                            return True, f"强趋势逆势做空: F&G={fng}, ADX={adx:.1f}"
